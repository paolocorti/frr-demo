...
export function WaypointMarker({ waypoint, pathId, isSelected, index }: WaypointMarkerProps) {
  ...
  // Drag handlers
  const handlePointerDown = useCallback((e: { stopPropagation: () => void; pointerId: number; target: { setPointerCapture: (id: number) => void } }) => {
    if (!isEditing) return;
    e.stopPropagation();
  ...
  const handlePointerMove = useCallback((e: { clientX: number; clientY: number }) => {
    ...
    const plane = new THREE.Plane(new THREE.Vector3(0, 1, 0), -waypoint.position[1]);
    ...
    if (point) {
      const newPos: [number, number, number] = [point.x, waypoint.position[1], point.z];
      updateWaypoint(pathId, waypoint.id, { position: newPos });
    }
  }, [isDragging, isEditing, waypoint.position, pathId, waypoint.id, updateWaypoint, camera, raycaster]);

  const handlePointerUp = useCallback((e: { pointerId: number; target: { releasePointerCapture: (id: number) => void } }) => {
    setIsDragging(false);
  ...
  return (
    <group>
      ...
      <Html position={[waypoint.position[0], waypoint.position[1] + 0.4, waypoint.position[2]]} center>
        <div style={{
      ...
      {isSelected && isEditing && (
        <Html position={[waypoint.position[0], waypoint.position[1] + 1.5, waypoint.position[2]]} center>
          <div
            ...
            <div style={{ color: '#888', fontSize: '10px', fontWeight: 'bold' }}>Position</div>
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              <input
                ...
                onChange={(e) => {
                  const newPos = [...waypoint.position] as [number, number, number];
            ...
            <div style={{ color: '#888', fontSize: '10px', fontWeight: 'bold', marginTop: '4px' }}>Look At (Camera Direction)</div>
            {(['X', 'Y', 'Z'] as const).map((axis, i) => (
              <div key={axis} style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const newLookAt = [...waypoint.lookAt] as [number, number, number];
