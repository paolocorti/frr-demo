...
interface CameraPathsState {
  ...
  // Waypoint actions
  addWaypoint: (pathId: string, position: [number, number, number], lookAt?: [number, number, number]) => void;
  removeWaypoint: (pathId: string, waypointId: string) => void;
  updateWaypoint: (pathId: string, waypointId: string, updates: Partial<Waypoint>) => void;
...
export const useCameraPathsStore = create<CameraPathsState>()((set) => ({
  ...
  updatePath: (id: string, updates: Partial<CameraPath>) => set((state) => ({
  ...
  // Waypoint actions
  addWaypoint: (pathId: string, position: [number, number, number], lookAt?: [number, number, number]) => {
    const waypoint = createWaypoint(position, lookAt);
    set((state) => ({
      paths: state.paths.map((p) =>
        p.id === pathId
          ? { ...p, waypoints: [...p.waypoints, waypoint] }
          : p
      ),
    }));
  },

  removeWaypoint: (pathId: string, waypointId: string) => set((state) => ({
    paths: state.paths.map((p) =>
      p.id === pathId
  ...
  updateWaypoint: (pathId: string, waypointId: string, updates: Partial<Waypoint>) => set((state) => ({
    paths: state.paths.map((p) =>
      p.id === pathId
        ? { ...p, waypoints: p.waypoints.map((w) =>
  ...
  setDraggingWaypoint: (dragging: boolean) => set({ isDraggingWaypoint: dragging }),

  // Persistence
  loadFromStorage: () => set({ paths: loadPathsFromStorage() }),
}));

// Auto-save to localStorage when paths change
useCameraPathsStore.subscribe(
  (state) => {
    try {
...
export const useActivePath = () =>
  useCameraPathsStore((state) =>
    state.paths.find((p) => p.id === state.activePathId) || null
  );
